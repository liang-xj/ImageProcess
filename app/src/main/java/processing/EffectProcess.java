...
public class EffectProcess {
    ...
    //构造方法
    public EffectProcess(Bitmap bmp)
    {
        mBitmap = bmp;
    }
    ...
    public Bitmap suMiaoImage(Bitmap bmp)
    {
        ...
        int width = bmp.getWidth();
        int height = bmp.getHeight();
        int[] pixels = discolor(bmp);    //存储灰度图像
        int[] copixels = reverseColor(pixels);     //反相
        gaussBlur(copixels, width, height, 10, 10/3);
    ...
    public static int[] discolor(Bitmap bitmap) {//去色
    ...
    public static int[] reverseColor(int[] pixels) {
        ...
        int[] inverse = new int[pixels.length];

        for (int i = 0, size = pixels.length; i < size; i++)
        {
            inverse[i] = 255 - pixels[i];
        }
        return inverse;
